...
public class PlagueMaskRenderer<L extends LivingEntity> implements ICurioRenderer {

    private static final ResourceLocation TEXTURE = new ResourceLocation(Hat.MOD_ID,
            "textures/cosmetics/plague_mask.png");

    private final PlagueMaskModel model;

    public PlagueMaskRenderer() {
        this.model = new PlagueMaskModel(Minecraft.getInstance().getEntityModels().bakeLayer(CosmeticLayerDefinitions.PLAGUE_MASK));
    }
