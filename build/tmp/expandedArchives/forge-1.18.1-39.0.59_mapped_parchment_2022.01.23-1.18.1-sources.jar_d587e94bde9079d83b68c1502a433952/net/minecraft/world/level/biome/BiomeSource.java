...
public abstract class BiomeSource implements BiomeResolver {
   ...
   protected BiomeSource(Stream<Supplier<Biome>> pPossibleBiomes) {
      this(pPossibleBiomes.map(Supplier::get).distinct().collect(ImmutableList.toImmutableList()));
   }

   protected BiomeSource(List<Biome> pPossibleBiomes) {
      this.possibleBiomes = new ObjectLinkedOpenHashSet<>(pPossibleBiomes);
      this.featuresPerStep = this.buildFeaturesPerStep(pPossibleBiomes, true);
   }
