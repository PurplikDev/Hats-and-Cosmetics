...
public class SocializeAtBell extends Behavior<LivingEntity> {
   ...
   protected boolean checkExtraStartConditions(ServerLevel pLevel, LivingEntity pOwner) {
      ...
      return pLevel.getRandom().nextInt(100) == 0 && optional.isPresent() && pLevel.dimension() == optional.get().dimension() && optional.get().pos().closerThan(pOwner.position(), 4.0D) && brain.getMemory(MemoryModuleType.NEAREST_VISIBLE_LIVING_ENTITIES).get().contains((p_24189_) -> {
