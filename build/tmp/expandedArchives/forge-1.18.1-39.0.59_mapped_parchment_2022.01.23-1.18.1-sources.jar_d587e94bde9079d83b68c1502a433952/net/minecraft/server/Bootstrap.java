...
public class Bootstrap {
   ...
   private static void validateThatAllBiomeFeaturesHaveBiomeFilter() {
      BuiltinRegistries.BIOME.stream().forEach((p_197754_) -> {
         List<List<Supplier<PlacedFeature>>> list = p_197754_.getGenerationSettings().features();
         list.stream().flatMap(Collection::stream).forEach((p_197756_) -> {
            if (!((PlacedFeature)p_197756_.get()).getPlacement().contains(BiomeFilter.biome())) {
               Util.logAndPauseIfInIde("Placed feature " + BuiltinRegistries.PLACED_FEATURE.getResourceKey((PlacedFeature)p_197756_.get()) + " is missing BiomeFilter.biome()");
            }
