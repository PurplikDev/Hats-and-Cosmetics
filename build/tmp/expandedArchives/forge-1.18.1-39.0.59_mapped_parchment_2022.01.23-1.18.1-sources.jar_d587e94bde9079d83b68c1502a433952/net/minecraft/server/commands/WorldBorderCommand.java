   ...
   private static int setSize(CommandSourceStack pSource, double pNewSize, long pTime) throws CommandSyntaxException {
      ...
      if (d0 == pNewSize) {
         ...
         if (pTime > 0L) {
            ...
            if (pNewSize > d0) {
               pSource.sendSuccess(new TranslatableComponent("commands.worldborder.set.grow", String.format(Locale.ROOT, "%.1f", pNewSize), Long.toString(pTime / 1000L)), true);
            } else {
               pSource.sendSuccess(new TranslatableComponent("commands.worldborder.set.shrink", String.format(Locale.ROOT, "%.1f", pNewSize), Long.toString(pTime / 1000L)), true);
            }
         } else {
