...
public class ScreenEffectRenderer {
   ...
   public static void renderScreenEffect(Minecraft pMinecraft, PoseStack pPoseStack) {
      ...
      if (!player.noPhysics) {
         ...
         if (overlay != null) {
            if (!net.minecraftforge.event.ForgeEventFactory.renderBlockOverlay(player, pPoseStack, net.minecraftforge.client.event.RenderBlockOverlayEvent.OverlayType.BLOCK, overlay.getLeft(), overlay.getRight()))
      ...
      if (!pMinecraft.player.isSpectator()) {
         if (pMinecraft.player.isEyeInFluid(FluidTags.WATER)) {
            if (!net.minecraftforge.event.ForgeEventFactory.renderWaterOverlay(player, pPoseStack))
            renderWater(pMinecraft, pPoseStack);
         }

         if (pMinecraft.player.isOnFire()) {
            if (!net.minecraftforge.event.ForgeEventFactory.renderFireOverlay(player, pPoseStack))
