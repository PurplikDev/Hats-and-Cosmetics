...
class NamespacedDefaultedWrapper<T extends IForgeRegistryEntry<T>> extends DefaultedRegistry<T> implements ILockableRegistry, IHolderHelperHolder<T>
{
    ...
    @Override
    public Holder<T> register(final ResourceKey<T> pKey, final T pValue, final Lifecycle pLifecycle)
    {
        return registerMapping(-1, pKey, pValue, pLifecycle);
    }

    @Override
    public Holder<T> registerOrOverride(final OptionalInt id, final ResourceKey<T> pKey, final T pValue, final Lifecycle pLifecycle)
    {
        int wanted = -1;
        if (id.isPresent() && byId(id.getAsInt()) != null)
            wanted = id.getAsInt();
        return registerMapping(wanted, pKey, pValue, pLifecycle);
    }
