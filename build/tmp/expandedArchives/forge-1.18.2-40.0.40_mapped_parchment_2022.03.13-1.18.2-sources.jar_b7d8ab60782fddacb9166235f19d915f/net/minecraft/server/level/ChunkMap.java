...
public class ChunkMap extends ChunkStorage implements ChunkHolder.PlayerProvider {
   ...
   private CompletableFuture<Either<ChunkAccess, ChunkHolder.ChunkLoadingFailure>> scheduleChunkGeneration(ChunkHolder pChunkHolder, ChunkStatus pChunkStatus) {
      ...
      CompletableFuture<Either<List<ChunkAccess>, ChunkHolder.ChunkLoadingFailure>> completablefuture = this.getChunkRangeFuture(chunkpos, pChunkStatus.getRange(), (p_203072_) -> {
         return this.getDependencyStatus(pChunkStatus, p_203072_);
      });
      this.level.getProfiler().incrementCounter(() -> {
         return "chunkGenerate " + pChunkStatus.getName();
      });
      Executor executor = (p_203100_) -> {
         this.worldgenMailbox.tell(ChunkTaskPriorityQueueSorter.message(pChunkHolder, p_203100_));
      };
      return completablefuture.thenComposeAsync((p_203016_) -> {
         return p_203016_.map((p_203022_) -> {
            try {
               CompletableFuture<Either<ChunkAccess, ChunkHolder.ChunkLoadingFailure>> completablefuture1 = pChunkStatus.generate(executor, this.level, this.generator, this.structureManager, this.lightEngine, (p_203062_) -> {
