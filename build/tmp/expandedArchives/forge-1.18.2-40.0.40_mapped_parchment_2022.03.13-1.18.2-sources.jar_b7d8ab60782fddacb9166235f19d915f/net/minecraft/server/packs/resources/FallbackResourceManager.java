...
public class FallbackResourceManager implements ResourceManager {
   ...
   public Resource getResource(ResourceLocation pResourceLocation) throws IOException {
      ...
      for(int i = this.fallbacks.size() - 1; i >= 0; --i) {
         ...
         if (packresources1.hasResource(this.type, pResourceLocation)) {
            InputStream inputstream = null;
            if (packresources != null) {
               inputstream = this.getWrappedResource(resourcelocation, packresources);
            }

            return new SimpleResource(packresources1.getName(), pResourceLocation, this.getWrappedResource(pResourceLocation, packresources1), inputstream);
         }
      }
