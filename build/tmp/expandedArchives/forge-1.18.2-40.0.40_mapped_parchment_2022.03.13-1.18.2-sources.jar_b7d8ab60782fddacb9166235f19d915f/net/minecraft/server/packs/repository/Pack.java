...
public class Pack implements AutoCloseable {
         ...
         label54: {
            try {
               ...
               if (packmetadatasection != null) {
                  pack = pFactory.create(pId, new TextComponent(packresources.getName()), pRequired, pSupplier, packmetadatasection, pDefaultPosition, pPackSource, packresources.isHidden());
                  break label54;
               }
   ...
   public Component getChatLink(boolean pGreen) {
      return ComponentUtils.wrapInSquareBrackets(this.packSource.decorate(new TextComponent(this.id))).withStyle((p_10441_) -> {
         return p_10441_.withColor(pGreen ? ChatFormatting.GREEN : ChatFormatting.RED).withInsertion(StringArgumentType.escapeIfRequired(this.id)).withHoverEvent(new HoverEvent(HoverEvent.Action.SHOW_TEXT, (new TextComponent("")).append(this.title).append("\n").append(this.description)));
      });
   }
