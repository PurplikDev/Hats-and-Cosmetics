...
public final class StructureStart {
   ...
   public StructureStart(ConfiguredStructureFeature<?, ?> pFeature, ChunkPos pChunkPos, int pReferences, PiecesContainer pPieceContainer) {
   ...
   public CompoundTag createTag(StructurePieceSerializationContext pContext, ChunkPos pChunkPos) {
      ...
      if (this.isValid()) {
         if (pContext.registryAccess().registryOrThrow(Registry.CONFIGURED_STRUCTURE_FEATURE_REGISTRY).getKey(this.getFeature()) == null) { // FORGE: This is just a more friendly error instead of the 'Null String' below
         ...
         compoundtag.putString("id", pContext.registryAccess().registryOrThrow(Registry.CONFIGURED_STRUCTURE_FEATURE_REGISTRY).getKey(this.feature).toString());
