...
public class SectionStorage<R> implements AutoCloseable {
   ...
   public SectionStorage(Path pFolder, Function<Runnable, Codec<R>> pCodec, Function<Runnable, R> pFactory, DataFixer pFixerUpper, DataFixTypes pType, boolean pSync, LevelHeightAccessor pLevelHeightAccessor) {
      this.codec = pCodec;
   ...
   private void readColumn(ChunkPos pChunkPos) {
      this.readColumn(pChunkPos, NbtOps.INSTANCE, this.tryRead(pChunkPos));
   }

   @Nullable
   private CompoundTag tryRead(ChunkPos pChunkPos) {
      try {
   ...
   private void writeColumn(ChunkPos pChunkPos) {
      Dynamic<Tag> dynamic = this.writeColumn(pChunkPos, NbtOps.INSTANCE);
      ...
      for(int i = this.levelHeightAccessor.getMinSection(); i < this.levelHeightAccessor.getMaxSection(); ++i) {
         ...
         if (optional != null && optional.isPresent()) {
            ...
            dataresult.resultOrPartial(LOGGER::error).ifPresent((p_63811_) -> {
               map.put(pOps.createString(s), p_63811_);
            });
         }
      }
