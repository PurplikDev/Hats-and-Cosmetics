...
public final class LevelStem {
   ...
   public static Registry<LevelStem> sortMap(Registry<LevelStem> pRegistry) {
      ...
      for(ResourceKey<LevelStem> resourcekey : BUILTIN_ORDER) {
         LevelStem levelstem = pRegistry.get(resourcekey);
         if (levelstem != null) {
            writableregistry.register(resourcekey, levelstem, pRegistry.lifecycle(levelstem));
         }
      }

      for(Entry<ResourceKey<LevelStem>, LevelStem> entry : pRegistry.entrySet()) {
         ResourceKey<LevelStem> resourcekey1 = entry.getKey();
         if (!BUILTIN_ORDER.contains(resourcekey1)) {
            writableregistry.register(resourcekey1, entry.getValue(), pRegistry.lifecycle(entry.getValue()));
         }
      }
   ...
   public static boolean stable(long pSeed, Registry<LevelStem> pRegistry) {
