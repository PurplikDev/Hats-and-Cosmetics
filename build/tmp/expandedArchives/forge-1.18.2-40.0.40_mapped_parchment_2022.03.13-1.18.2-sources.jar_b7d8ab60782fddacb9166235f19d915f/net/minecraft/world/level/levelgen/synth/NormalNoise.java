...
public class NormalNoise {
   ...
   /** @deprecated */
   @Deprecated
   public static NormalNoise createLegacyNetherBiome(RandomSource pRandomSource, NormalNoise.NoiseParameters pParameters) {
      return new NormalNoise(pRandomSource, pParameters, false);
   }

   public static NormalNoise create(RandomSource pRandomSource, int pOctaves, double... pAmplitudes) {
      return create(pRandomSource, new NormalNoise.NoiseParameters(pOctaves, new DoubleArrayList(pAmplitudes)));
   }

   public static NormalNoise create(RandomSource pRandomSource, NormalNoise.NoiseParameters pPrameters) {
      return new NormalNoise(pRandomSource, pPrameters, true);
   }

   private NormalNoise(RandomSource pRandomSource, NormalNoise.NoiseParameters pParameters, boolean pUseLegacyNetherBiome) {
      int i = pParameters.firstOctave;
      DoubleList doublelist = pParameters.amplitudes;
      this.parameters = pParameters;
      if (pUseLegacyNetherBiome) {
         this.first = PerlinNoise.create(pRandomSource, i, doublelist);
         this.second = PerlinNoise.create(pRandomSource, i, doublelist);
      } else {
         this.first = PerlinNoise.createLegacyForLegacyNetherBiome(pRandomSource, i, doublelist);
         this.second = PerlinNoise.createLegacyForLegacyNetherBiome(pRandomSource, i, doublelist);
      }
